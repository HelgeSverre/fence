...
progressSuite =
    let
        ...
        ( fresh, _ ) =
            Markdown.begin Markdown.emptyCache Markdown.noDocument doc
    ...
    describe "progressive parsing"
        ...
        , test "cached chunks are free: an edit to one chunk finishes in one small step" <|
            \_ ->
                let
                    ...
                    ( again, _ ) =
                        Markdown.begin cache Markdown.noDocument (doc ++ " edited")
                in
        ...
        , test "unchanged chunks keep the identical rendered value across a re-parse" <|
            \_ ->
                let
                    ...
                    ( again, _ ) =
                        Markdown.begin (Markdown.cache first) Markdown.noDocument (doc ++ " edited")
                in
        ...
        , test "a duplicate heading inserted earlier re-renders later chunks with new ids" <|
            \_ ->
                let
                    ...
                    ( again, _ ) =
                        Markdown.begin (Markdown.cache first) Markdown.noDocument ("# H3\n\nintro\n\n" ++ doc)
                in
        ...
        , test "an HTML document that cannot be split renders as one chunk" <|
            \_ ->
                Markdown.begin Markdown.emptyCache Markdown.noDocument "<div>\n\n# A\n\n# B\n\n</div>"
                    |> Tuple.first
                    |> runToEnd
                    |> Markdown.htmlChunks
                    |> List.length
